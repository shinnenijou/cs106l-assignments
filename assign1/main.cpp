...
struct Course {
  ...
  Course(std::string title, std::string number_of_units, std::string quarter):
    ...
    number_of_units(std::move(number_of_units)),
    quarter(std::move(quarter)) {}
};
...
void parse_csv(const std::string& filename, std::vector<Course>& courses) {
  ...
  const auto lines = split(content_stream.str(), '\n');

  for (int i = 1; i < lines.size(); i++)
  {
    auto course_infos = split(lines[i], ',');
    courses.emplace_back(course_infos[0], course_infos[1], course_infos[2]);
  }
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  std::ofstream file(COURSES_OFFERED_PATH, std::ios::out | std::ios::binary);
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  std::ofstream file(COURSES_NOT_OFFERED_PATH, std::ios::out | std::ios::binary);
